...
public class WaterEntry extends GUIEntry {

    public WaterEntry(Material material, String name, List<String> lore, int quantity, int slot) {
    ...
    public void onClick(InventoryClickEvent event) {
        ...
        if (CoreMain.getInstance().getSql().isConnected() && CoreMain.getInstance().getSqlPlayerData().exists(uuid)) {
            data.setElement(uuid, "&3&lWaterElement");
        } else {
        ...
        msgPlayer(player, msgUtil.getCReplaceMessage(MessageUtils.Message.DIVINEPLAYER) + color(" &f&lYou now have the " + "&3&lWaterElement"));
