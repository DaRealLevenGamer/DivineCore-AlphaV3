...
public class EarthEntry extends GUIEntry {

    public EarthEntry(Material material, String name, List<String> lore, int quantity, int slot) {
    ...
    public void onClick(InventoryClickEvent event) {
        ...
        if (CoreMain.getInstance().getSql().isConnected() && CoreMain.getInstance().getSqlPlayerData().exists(uuid)) {
            data.setElement(uuid, "&a&lEarthElement");
        } else {
        ...
        msgPlayer(player, msgUtil.getCReplaceMessage(MessageUtils.Message.DIVINEPLAYER) + color(" &f&lYou now have the " + "&a&lEarthElement"));
